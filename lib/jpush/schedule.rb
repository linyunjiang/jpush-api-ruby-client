...
module JPush
  ...
  module Schedule
    ...
    def create(schedule_payload)
      ...
      Http::Client.post(@config_settings, @base_url, body: body)
    end

    # GET https://api.jpush.cn/v3/schedules?page=
    # 获取当前有效（endtime未过期）的 schedule 列表
    def tasks(page = nil)
      Http::Client.get(@config_settings, @base_url, params: { page: page })
    end

    # 获取指定的定时任务
    # GET https://api.jpush.cn/v3/schedules/{schedule_id}
    def show(schedule_id)
      Http::Client.get(@config_settings, @base_url + schedule_id)
    end
    ...
    def update(schedule_id, name: nil, enabled: nil, trigger: nil, push: nil)
      ...
      Http::Client.put(@config_settings, @base_url + schedule_id, body: body)
    end

    # 删除指定的Schedule任务
    # DELETE https://api.jpush.cn/v3/schedules/{schedule_id}
    def delete(schedule_id)
      Http::Client.delete(@config_settings, @base_url + schedule_id)
    end
    ...
    def base_url(settings)
      ...
      @base_url = settings[:push_api_host] + settings[:api_version] + '/schedules/'
    end
